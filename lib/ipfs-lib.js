...
class IPFS {
  constructor (ipfsConfig) {
    ...
    this.pubsub = new PubSub({ ipfs: this.ipfs })
  ...
  async managePubSubChannels () {
    try {
      ...
      this.pubsub.subscribeToPubsubChannel(DEFAULT_COORDINATION_ROOM)
  ...
  async manageAnnouncement () {
    try {
      ...
      await this.pubsub.publishToPubsubChannel(DEFAULT_COORDINATION_ROOM, testStr)

      this.logger('manageAnnouncement')
    } catch (err) {
