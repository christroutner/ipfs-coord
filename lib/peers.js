...
class Peer {
  ...
  async addPeer (announceObj) {
    try {
      ...
      if (!this.state.peers[announceObj.from]) {
        console.log(`New peer found: ${announceObj.from}`)
        this.statusLog(`New peer found: ${announceObj.from}`)

        this.state.peerList.push(announceObj.from.toString())
      ...
      this.state.peers[announceObj.from] = announceObj.data
